...
We should not consider models which include both X1 and X2.
...
```{R}
stu_res <- studres(backward.step.model)
plot(backward.step.model$fitted.values,stu_res)
qqnorm(stu_res)
qqline(stu_res)
```

Visually, 2/5 data points are outliers with large residuals.
...
```{R}
data6 <- fread('https://raw.githubusercontent.com/wilsonify/AppliedRegression/master/data/wide_release_moveies.csv')
```
